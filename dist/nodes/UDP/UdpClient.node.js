...
async function sendUdpData(host, port, message, waitForResponse, responseTimeout, encoding) {
    ...
    async execute() {
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                const result = await sendUdpData(host, port, message, waitForResponse, responseTimeout, encoding);
                returnData.push({
                    json: result,
                });
            }
